...
public class HamcrestMatchersApiTest {
    ...
    */
    ...
    public void OneSpartanWithHamcrest(){
        ...
        //chaining method(chaining the request)

        given().accept(ContentType.JSON)
                .and().pathParam("id", 15)
        .when().get("http://3.87.241.127:8000/api/spartans/{id}")
                .then().statusCode(200)

//                .and().assertThat().contentType("application/json")
//                .and().assertThat().contentType("application")  //it doesnt accept the exact match !!
                .and().assertThat().contentType(equalTo("application/json")) // o yuzden equals to koysak exact match i assert ederiz
...
//                .when().get("http://api.cybertektraining.com/teacher/{id}")
                ...
                .and().header("Content-Length", equalTo("292"))
                .and().header("Connection", equalTo("Keep-Alive"))
                .and().header("Date", notNullValue())
                .and().assertThat().body("teachers.firstName[0]", equalTo("William"),
